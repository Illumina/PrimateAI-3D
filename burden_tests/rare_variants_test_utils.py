...
#
#     filtered_rv = filtered_rv[[GENE_NAME, VCF_AC, ALL_SAMPLES, PRIMATEAI_SCORE]].copy()
#     filtered_rv[PRIMATEAI_SCORE] = filtered_rv[PRIMATEAI_SCORE].fillna(1)
#
#     results = None
#
#     if test_meds:
...
#         batch_size = int(len(all_genes) / n_threads)
#
#         batch_genes = [all_genes[i * batch_size:
#                                  ((i + 1) * batch_size if i < n_threads - 1 else len(all_genes))]
#                        for i in range(n_threads)]
#
#         echo('Batch sizes:', list(map(len, batch_genes)), ', total=', sum(map(len, batch_genes)))
#         ph_data_fname = dump_to_tmp_file(ph_data.copy(), output_dir=output_dir)
#
...
#             batch_results_fnames = pool.map(compute_results, batch_params)
#             echo('Batch results:', batch_results_fnames)
#
#             batch_results = list(map(load_from_tmp_file, batch_results_fnames))
#             # batch_results = list(map(compute_results, batch_params))
#
...
#         else:
#
#             results = filtered_rv.groupby(GENE_NAME).apply(rv_test_gene,
#                                                            ph_data,
#                                                            ph_name,
#                                                            key_prefix,
#                                                            sort_by_pvalue_label,
...
#                                                            multiple_testing_correction,
#                                                            is_binary
#                                                            ).sort_values(key_prefix + '/' + sort_by_pvalue_label + '/pvalue/fdr_corr').reset_index()
#
#     return results
...
def process_rv_test_batch_randomized(batch_params):
    ...
    if test_meds:
        ...
        for batch_idx, genes in enumerate(batch_genes):
            ...
            if is_binary:
                # rv_test_for_batch_binary(genes_variants,
                #                          MAX_pAI_THRESHOLDS_TO_TEST,
                #                          find_best_AC_threshold,
                #                          find_best_pAI_threshold,
                #                          key_prefix,
                ...
                #                          results,
                #                          test_type=LOGIT,
                #                          const=ph_data[CONST_LABEL].to_numpy(),
...
#                          pathogenicity_score_label=None):
#
#     carrier_table = {SAMPLE_ID: ph_data[SAMPLE_ID]}
#
#     genes = sorted(set(filtered_rv[GENE_NAME]))
#
#     echo('genes=', len(genes))
...
#
#         echo(gene, ', n_tests_key:', n_tests_key)
#         ph_values = ph_data[ph_name]
#
#         if phenotype_name_original is not None:
#             ph_original_values = ph_data[phenotype_name_original]
#         else:
#             ph_original_values = ph_values
#
#         all_samples = set(ph_data[SAMPLE_ID])
#
#         var_to_carriers = dict((row[VARID_REF_ALT],
#                                 set(row[ALL_SAMPLES].split(',')) & all_samples
#                                 ) for _, row in gene_variants.iterrows())
#
#         sample_id_to_idx = dict((sid, idx) for idx, sid in enumerate(ph_data[SAMPLE_ID]))
#
#         base_carrier_status = np.array([0] * len(ph_data))
#
#         seen = set()
#         for ac_threshold in ac_thresholds:
#             c_vars_by_ac = gene_variants[(gene_variants[VCF_AC] <= ac_threshold)]
...
#             for pAI_threshold in pAI_thresholds:
#
#                 c_vars = c_vars_by_ac[c_vars_by_ac[pathogenicity_score_label] >= pAI_threshold]
#                 n_vars = len(c_vars)
#
#                 varids = c_vars[VARID_REF_ALT]
#
#                 seen_key = tuple(sorted(varids))
#                 if seen_key in seen:
#                     continue
#                 seen.add(seen_key)
#
#                 carrier_status = base_carrier_status.copy()
#
#                 for v in varids:
#                     for sid in var_to_carriers[v]:
#                         carrier_status[sample_id_to_idx[sid]] = 1
#
#                 n_carriers = int(np.sum(carrier_status))
#                 n_non_carriers = len(ph_data) - n_carriers
#
#                 # carrier_status = ph_data[SAMPLE_ID].isin(carriers).astype(int)
#
#                 beta = np.sum(carrier_status * ph_values) / n_carriers - np.sum((1 - carrier_status) * ph_values) / n_non_carriers
#                 beta_in_original_units = np.sum(carrier_status * ph_original_values) / n_carriers - np.sum((1 - carrier_status) * ph_original_values) / n_non_carriers
#
#                 # beta = 0
#                 col_name = ';'.join([gene_name_key,
#                                      key_value_to_string('beta', float(beta)),
#                                      key_value_to_string('beta_in_original_units', float(beta_in_original_units)),
#                                      key_value_to_string('n_carriers', n_carriers),
#                                      key_value_to_string('n_variants', n_vars),
#                                      key_value_to_string('best_AC', int(ac_threshold)),
...
#                                      ])
#
#                 carrier_table[col_name] = carrier_status
#
#     echo('Creating DataFrame')
#     carrier_table = pd.DataFrame(carrier_table)
#
#     echo('Merging with ph_data')
#     carrier_table = pd.merge(ph_data, carrier_table, on=SAMPLE_ID)
#
...
# def rv_test_for_batch_v2(genes_variants,
#                          MAX_pAI_THRESHOLDS_TO_TEST,
#                          find_best_AC_threshold,
#                          find_best_pAI_threshold,
#                          key_prefix,
...
#                          test_type=RANKSUM_TEST,
#                          full_random_stats=False):
#
#     echo('rv_test_for_batch v2!!')
#
#     gene_names = sorted(genes_variants[GENE_NAME].unique())
#     all_samples = set(sid_to_phenotypes.keys())
#
#     for gene_name in gene_names:
#         cur_gene_variants = genes_variants[genes_variants[GENE_NAME] == gene_name]
#
...
#
#             n_non_carriers = len(all_samples) - n_carriers
#
...
#                 non_carrier_var = (n_non_carriers * non_carrier_total_of_squares - np.square(non_carrier_totals)) / (
#                             n_non_carriers ** 2)
#
#                 df = len(all_samples) - 2
#
...
#
#         for ph_idx in range(n_phenotypes):
...
#             append_value(results, key_prefix + '/carrier/pvalue/fdr_corr', best_combo_fdr)
#
#         if full_random_stats:
#             for c in random_cols:
#                 append_value(results, key_prefix + '/random/' + c, random_cols[c])
#         else:
#             append_value(results, key_prefix + '/random/more_significant', np.sum(np.array(random_cols['carrier/pvalue']) <= best_real_pvalue))
#             append_value(results, key_prefix + '/random/n_randomizations', n_phenotypes - 1)
#

# def rv_test_for_batch_v3(genes_variants,
#                          MAX_pAI_THRESHOLDS_TO_TEST,
#                          find_best_AC_threshold,
#                          find_best_pAI_threshold,
#                          key_prefix,
...
#                          skip_randomizing_intermediate_significant=False):
#
#     echo('[rv_test_for_batch v3], adaptive_fdr:', adaptive_fdr, ', MAX_N_RANDOMIZATIONS', MAX_N_RANDOMIZATIONS, ', test_type:', test_type)
#
#     rnd_state = random.getstate()
#     np_rnd_state = np.random.get_state()
#
...
#     all_samples = set(sid_to_phenotypes.keys())
#
#     original_sid_to_phenotypes = sid_to_phenotypes.copy()
#
#     for gene_name in gene_names:
#
#         random.setstate(rnd_state)
#         np.random.set_state(np_rnd_state)
#
#         sid_to_phenotypes = original_sid_to_phenotypes
#
#         cur_gene_variants = genes_variants[genes_variants[GENE_NAME] == gene_name]
#
#         keep_testing = True
...
#                         if 1e-5 < best_combo_fdr <= 1e-2 and adaptive_fdr:
#                             append_value(results, key_prefix + '/carrier/pvalue/fdr_corr', best_combo_fdr)
...
#                             break
#
#                     if best_combo_fdr <= 1e-5 or not adaptive_fdr:
#                         append_value(results, key_prefix + '/carrier/pvalue/fdr_corr', best_combo_fdr)
#                         append_value(results, key_prefix + '/carrier/pvalue/n_randomizations', 0)
...
#                 n_randomizations += n_phenotypes - 1
...
#
#                     if c_fdr == 0:
#                         echo('c_fdr was estimated to be 0, reverting to BH FDR:', best_combo_fdr)
#                         c_fdr = best_combo_fdr
#
...
#                 else:
#
#                     ph_values = np.tile(ph_data[ph_name], (n_phenotypes, 1))
#
#                     for i in range(1, n_phenotypes):
#                         np.random.shuffle(ph_values[i])
#
#                     ph_values = ph_values.T
#
#                     sid_to_phenotypes = {}
#                     for sid_idx, sid in enumerate(list(ph_data[SAMPLE_ID])):
#                         sid_to_phenotypes[sid] = ph_values[sid_idx, :]
