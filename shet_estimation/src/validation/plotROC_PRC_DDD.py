...
'''
USAGE
The command to run the script for plotting ROC and PRC curves for validation data set DDD is :
python  /path/to/source/validation/plotROC_PRC_DDD.py \
     [datafile] 
...
dd=pd.read_csv('DDgene.list.txt',header=None)
ndd=pd.read_csv('nonDDgene.list.txt',header=None)
dd.columns=['genename']
ndd.columns=['genename']
dd['label']=1
ndd['label']=0
ddd=pd.concat([dd,ndd],ignore_index=True)

df1=ddd.merge(d1,how='left',on='genename')
...
roc_auc_df.to_csv('auROC_PRC.csv',index=False)


###plot ROC Curves 
roc_auc_df=pd.read_csv('auROC_PRC.csv',header=0)
...
for metric_index,metric in enumerate(metrics1):
    ...
    pr_auc = auc(recall, precision)
    metric_name=labels1[metric_index]
    line1, =ax.plot(recall, precision, color=color1[metric_index], label=f'{metric_name}  (auPRC = {pr_auc:.4f})')
    list1.append(line1)
    print(pr_auc)
...
for metric_index,metric in enumerate(metrics2):
    ...
    pr_auc = auc(recall, precision)
    metric_name=labels2[metric_index]
    line2, =ax.plot(recall, precision, color=color2[metric_index], label=f'{metric_name}  (auPRC = {pr_auc:.4f})')
    list2.append(line2)

l1=ax.legend(handles=list1, loc=(0.06,0.01), labelcolor=color1, title='Constraint metrics (other):',alignment='left')
ax.add_artist(l1)
l2=ax.legend(handles=list2, loc=(0.06,0.28), labelcolor=color2,title='Constraint metrics (this work):',alignment='left')
plt.xlabel('Recall')
